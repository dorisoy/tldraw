		...
		for (const { sequence, versionAtInstallation } of sequences) {
			...
			for (const unusedMigrationId of unusedMigrationIds) {
				if (!unusedMigrationId.startsWith(sequence.id) + '/') {
					throw new Error(
						`Migration id ${JSON.stringify(unusedMigrationId)} must start with ${JSON.stringify(sequence.id)}`
					)
				}
				...
				if (order.includes(unusedMigrationId)) {
					throw new Error(
						`Unused migration id ${JSON.stringify(unusedMigrationId)} is present in your migration order. Did you specify 'versionAtInstallation' correctly?`
					)
				}
			...
			for (const migration of usedMigrations) {
				if (!migration.id.startsWith(sequence.id) + '/') {
					throw new Error(
						`Migration id ${JSON.stringify(migration.id)} must start with ${JSON.stringify(sequence.id)}`
					)
				}
				...
				if (orderIdx === -1) {
					...
					throw new Error(
						`Migration id ${JSON.stringify(migration.id)} is out of order. It should come after ${JSON.stringify(order[lastIdx])}`
					)
				} else {
					lastIdx = orderIdx
				}
...
`
		...
		for (let i = 0; i < this.sortedMigrationIds.length; i++) {
			...
			if (!migration) {
				...
				throw new Error(
					`Missing migration details for ${JSON.stringify(id)}. Did you forget to add a migration sequence?`
				)
			}
